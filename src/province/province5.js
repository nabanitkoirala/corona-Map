...
function Province5() {
    ...
    const [Province5total, setProvince5total] = useState([]);
    const [Prov5dist, setProv5dist] = useState([]);//districts of Bagmati state
    const [Province5dubs, setProvince5dubs] = useState([]);//duplicate data of bagmati fetched from api for seleccting district form province
    ...
    useEffect(() => {
        axios.get('https://data.nepalcorona.info/api/v1/covid')
            .then(res => {
                setProvince5(res.data.filter((item) => item.province === 5))
                setProvince5dubs(res.data.filter((item) => item.province === 5))
            })
            .catch(err => {
                console.log(err);
            })
        ...
        axios.get('https://data.nepalcorona.info/api/v1/covid/summary')
            .then(res => {
                setProvince5total([res.data.province])
        ...
        axios.get('https://data.nepalcorona.info/api/v1/districts')
            ...
            .then(res => {
                setProv5dist(res.data.filter((item) => item.province === 5))
    ...
    console.log("Province5 data>>>", Province5);
    console.log("Province5 total>>", Province5total);
    return (
        <div>
            <Header />
            <div>
                        ...
                        <div className="container-fluid">
                            ...
                            <div className="row">
                                                ...
                                                <div className="p-1 flex-fill" style={{ overflow: 'hidden' }}>
                                                    ...
                                                    <div id="world-map-markers" style={{ height: 600, overflow: 'hidden' }}>
                                                        ...
                                                        <ReactMapGl className="map-react" {...viewPort} mapboxApiAccessToken="pk.eyJ1IjoibmFiYW5pdCIsImEiOiJja2E4MXR3NDkwNGMxMzJzOWF4Zzk1cmRxIn0.PlAUDME-BUb9gV-DCutXzw" mapStyle="mapbox://styles/nabanit/ckagghg6a0jfi1il7gdyjpt4c"
                                                            ...
                                                            {Province5.filter((local) => local.currentState === 'recovered').map((local) => (
                                                            ...
                                                            {Province5.filter((local) => local.currentState === 'active').map((local) => (
                                                                <Marker key={local.id} latitude={local.point.coordinates[1]} longitude={local.point.coordinates[0]}>
                                                                    <button className="marker-btn" onClick={e => {
                                                                        e.preventDefault();
                                                                        setselectedPerson(local);
                                                            ...
                                                            {Province5.filter((local) => local.currentState === 'death').map((local) => (
                                                                <Marker key={local.id} latitude={local.point.coordinates[1]} longitude={local.point.coordinates[0]}>
                                ...
                                <div className="col-md-4">
                                    <h3 className="text-center"><strong><u>Province 5</u></strong></h3>
                                    {/* Info Boxes Style 2 */}
                                    <div className="info-box mb-3 bg-warning">
                                        <span className="info-box-icon"><i className="fas fa-lungs-virus" /></span>
                                        <div className="info-box-content">
                                            <span className="info-box-text">Infected</span>
                                            <span className="info-box-number">{Province5total.map((item) => item.active[3].count)}</span>
                                        </div>
                                        {/* /.info-box-content */}
                                    </div>
                                    {/* /.info-box */}
                                    ...
                                    <div className="info-box mb-3 bg-info">
                                        ...
                                        <div className="info-box-content">
                                            ...
                                            <span className="info-box-number">{Province5total.map((item) => item.cases[1].count)}</span>
                                        </div>
                                        {/* /.info-box-content */}
                                    </div>
                    ...
                    <div className="form-group">
                        ...
                        <select className="form-control" onChange={(e) => {
                            if (e.target.value === "Overall") {
                                return (setProvince5(Province5dubs))
                            }
                            else {
                                return (setProvince5(Province5dubs.filter((item) => item.district === Number(e.target.value))))
                            }
                            ...
                            {Prov5dist.map((item, _id) => <option key={_id} value={item.id}>{item.title_ne}</option>)}
                        </select>
                    </div>
                </div>
            </aside >
