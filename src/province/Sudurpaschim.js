...
function Sudurpaschim() {
    ...
    const [Sudurpaschim, setSudurpaschim] = useState([]);
    const [Sudurpaschimtotal, setSudurpaschimtotal] = useState([]);
    const [Sudurdist, setSudurdist] = useState([]);//districts of Bagmati state
    const [Sudurpaschimdubs, setSudurpaschimdubs] = useState([]);//duplicate data of bagmati fetched from api for seleccting district form province
    ...
    useEffect(() => {
        axios.get('https://data.nepalcorona.info/api/v1/covid')
            .then(res => {
                setSudurpaschim(res.data.filter((item) => item.province === 7))
                setSudurpaschimdubs(res.data.filter((item) => item.province === 7))
            })
            .catch(err => {
                console.log(err);
            })
        ...
        axios.get('https://data.nepalcorona.info/api/v1/covid/summary')
            .then(res => {
                setSudurpaschimtotal([res.data.province])
        ...
        axios.get('https://data.nepalcorona.info/api/v1/districts')
            ...
            .then(res => {
                setSudurdist(res.data.filter((item) => item.province === 7))
    ...
    console.log("Sudurpaschim data>>>", Sudurpaschim);
    console.log("Sudurpaschim total>>", Sudurpaschimtotal);
    return (
        <div>
            <Header />
            <div>
                        ...
                        <div className="container-fluid">
                            ...
                            <div className="row">
                                                ...
                                                <div className="p-1 flex-fill" style={{ overflow: 'hidden' }}>
                                                    ...
                                                    <div id="world-map-markers" style={{ height: 600, overflow: 'hidden' }}>
                                                        ...
                                                        <ReactMapGl className="map-react" {...viewPort} mapboxApiAccessToken="pk.eyJ1IjoibmFiYW5pdCIsImEiOiJja2E4MXR3NDkwNGMxMzJzOWF4Zzk1cmRxIn0.PlAUDME-BUb9gV-DCutXzw" mapStyle="mapbox://styles/nabanit/ckagghg6a0jfi1il7gdyjpt4c"
                                                            ...
                                                            {Sudurpaschim.filter((local) => local.currentState === 'recovered').map((local) => (
                                                            ...
                                                            {Sudurpaschim.filter((local) => local.currentState === 'active').map((local) => (
                                                                <Marker key={local.id} latitude={local.point.coordinates[1]} longitude={local.point.coordinates[0]}>
                                                                    <button className="marker-btn" onClick={e => {
                                                                        e.preventDefault();
                                                                        setselectedPerson(local);
                                                            ...
                                                            {Sudurpaschim.filter((local) => local.currentState === 'death').map((local) => (
                                                                <Marker key={local.id} latitude={local.point.coordinates[1]} longitude={local.point.coordinates[0]}>
                                ...
                                <div className="col-md-4">
                                    <h3 className="text-center"><strong><u>Sudurpaschim</u></strong></h3>
                                    {/* Info Boxes Style 2 */}
                    ...
                    <div className="form-group">
                        ...
                        <select className="form-control" onChange={(e) => {
                            if (e.target.value === "Overall") {
                                return (setSudurpaschim(Sudurpaschimdubs))
                            }
                            else {
                                return (setSudurpaschim(Sudurpaschimdubs.filter((item) => item.district === Number(e.target.value))))
                            }

                        }} >
                            <option >Choose District</option>
                            <option value="Overall">Overall</option>
                            {Sudurdist.map((item, _id) => <option key={_id} value={item.id}>{item.title_ne}</option>)}
                        </select>
                    </div>
                </div>
            </aside >
