...
function Gandaki() {
    ...
    const [Gandaki, setGandaki] = useState([]);
    const [Gandakitotal, setGandakitotal] = useState([]);
    const [Gandist, setGandist] = useState([]);//districts of Bagmati state
    const [Gandakidubs, setGandakidubs] = useState([]);//duplicate data of bagmati fetched from api for seleccting district form province
    ...
    useEffect(() => {
        axios.get('https://data.nepalcorona.info/api/v1/covid')
            .then(res => {
                setGandaki(res.data.filter((item) => item.province === 4))
                setGandakidubs(res.data.filter((item) => item.province === 4))
            })
            .catch(err => {
                console.log(err);
            })
        ...
        axios.get('https://data.nepalcorona.info/api/v1/covid/summary')
            .then(res => {
                setGandakitotal([res.data.province])
        ...
        axios.get('https://data.nepalcorona.info/api/v1/districts')
            ...
            .then(res => {
                setGandist(res.data.filter((item) => item.province === 4))
    ...
    console.log("Gandaki data>>>", Gandaki);
    console.log("Gandaki total>>", Gandakitotal);
    return (
        <div>
            <Header />
            <div>
                        ...
                        <div className="container-fluid">
                            ...
                            <div className="row">
                                                ...
                                                <div className="p-1 flex-fill" style={{ overflow: 'hidden' }}>
                                                    ...
                                                    <div id="world-map-markers" style={{ height: 600, overflow: 'hidden' }}>
                                                        ...
                                                        <ReactMapGl className="map-react" {...viewPort} mapboxApiAccessToken="pk.eyJ1IjoibmFiYW5pdCIsImEiOiJja2E4MXR3NDkwNGMxMzJzOWF4Zzk1cmRxIn0.PlAUDME-BUb9gV-DCutXzw" mapStyle="mapbox://styles/nabanit/ckagghg6a0jfi1il7gdyjpt4c"
                                                            ...
                                                            {Gandaki.filter((local) => local.currentState === 'recovered').map((local) => (
                                                            ...
                                                            {Gandaki.filter((local) => local.currentState === 'active').map((local) => (
                                                                <Marker key={local.id} latitude={local.point.coordinates[1]} longitude={local.point.coordinates[0]}>
                                                                    <button className="marker-btn" onClick={e => {
                                                                        e.preventDefault();
                                                                        setselectedPerson(local);
                                                            ...
                                                            {Gandaki.filter((local) => local.currentState === 'death').map((local) => (
                                                                <Marker key={local.id} latitude={local.point.coordinates[1]} longitude={local.point.coordinates[0]}>
                                ...
                                <div className="col-md-4">
                                    <h3 className="text-center"><strong><u>Gandaki</u></strong></h3>
                                    {/* Info Boxes Style 2 */}
                                    <div className="info-box mb-3 bg-warning">
                                        <span className="info-box-icon"><i className="fas fa-lungs-virus" /></span>
                                        <div className="info-box-content">
                                            <span className="info-box-text">Infected</span>
                                            <span className="info-box-number">{Gandakitotal.map((item) => item.active[1].count)}</span>
                                        </div>
                                        {/* /.info-box-content */}
                                    </div>
                                    {/* /.info-box */}
                                    ...
                                    <div className="info-box mb-3 bg-info">
                                        ...
                                        <div className="info-box-content">
                                            ...
                                            <span className="info-box-number">{Gandakitotal.map((item) => item.cases[2].count)}</span>
                                        </div>
                                        {/* /.info-box-content */}
                                    </div>
                    ...
                    <div className="form-group">
                        ...
                        <select className="form-control" onChange={(e) => {
                            if (e.target.value === "Overall") {
                                return (setGandaki(Gandakidubs))
                            }
                            else {
                                return (setGandaki(Gandakidubs.filter((item) => item.district === Number(e.target.value))))
                            }
                            ...
                            {Gandist.map((item, _id) => <option key={_id} value={item.id}>{item.title_ne}</option>)}
                        </select>
                    </div>
                </div>
            </aside >
